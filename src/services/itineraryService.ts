import type { ActivityCategory, CountryInfo, Trip, Activity} from "../models.ts";
import {
    getTripFromDatabase,
...
export const getActivitiesByDate = async (tripId: string, date: Date): Promise<Activity[]> => {
    try {
        const trip = await getTripFromDatabase(tripId);
        if(!trip) {
        ...
        const activities = trip.activities.filter(activity => {
            const activityDate = activity.startTime instanceof Date ? activity.startTime : new Date(activity.startTime);
            return activityDate.toDateString() === selectedDate;
        });

        return activities;
    } catch (error: unknown) {
        throw new Error('Could find activities at the specific date', { cause: error });
    }
    }